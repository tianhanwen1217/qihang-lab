...
const cancelFileUpload = async (shareId: string, fileId: string) => {
  await api.delete(`shares/${shareId}/files/${fileId}/upload`);
};
...
const updateFileAccess = async (
  ...
  (await api.patch(`shares/${shareId}/files/${fileId}/access`, settings)).data;
