...
export class S3FileService {
  ...
  async create(
    ...
    try {
      ...
      if (chunk.index === chunk.total - 1) {
        ...
        multipartUpload.completed = true;
      }
    } catch (error) {
  ...
  async get(shareId: string, fileId: string): Promise<File> {
    ...
    return {
      metaData: {
        ...
        mimeType: mime.contentType(fileName) || "application/octet-stream",
      },
  ...
  async cancelUpload(shareId: string, fileId: string) {
    ...
    const multipartUpload = this.multipartUploads[fileId];
    ...
    delete this.multipartUploads[fileId];
  }
      ...
      while (true) {
        ...
        await s3Instance.send(
          ...
          if (result.Errors?.length) {
            throw new Error(
              `Could not delete ${result.Errors.length} S3 objects`,
            );
          }
        });
      }
          ...
          try {
            ...
            if (response.Body instanceof Readable) {
              ...
              archive.append(fileStream, {
                name: safeArchiveEntryPath(fileName, `file-${index + 1}`),
              });
            } else {
              processNextFile(index + 1);
            }
