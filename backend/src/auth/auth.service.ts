...
export class AuthService {
  ...
  async resetPassword(token: string, newPassword: string) {
    ...
    const user = await this.prisma.user.findFirst({
      where: {
        resetPasswordToken: { token, expiresAt: { gt: new Date() } },
      },
    });
    ...
    await this.prisma.$transaction([
      this.prisma.resetPasswordToken.delete({ where: { token } }),
      this.prisma.refreshToken.deleteMany({ where: { userId: user.id } }),
      this.prisma.loginToken.deleteMany({ where: { userId: user.id } }),
  ...
  async refreshAccessToken(refreshToken: string) {
    ...
    if (
      ...
      !refreshTokenMetaData.user.isActive ||
      refreshTokenMetaData.expiresAt < new Date()
    )
