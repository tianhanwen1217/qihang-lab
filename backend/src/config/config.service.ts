...
export class ConfigService extends EventEmitter {
  ...
  async getByCategory(category: string) {
    const visibleKeys: Record<string, string[]> = {
      general: ["appName", "appUrl", "sessionDuration"],
      share: [
        "shareIdLength",
        "maxSize",
        "zipCompressionLevel",
    ...
    if (!visibleKeys[category]) {
      throw new NotFoundException("Configuration category not found");
    }
    const configVariables = this.configVariables
      .filter(
        (c) =>
          !c.locked &&
          category == c.category &&
          visibleKeys[category].includes(c.name),
      )
      .sort((c) => c.order);
  ...
  async updateMany(data: { key: string; value: string | number | boolean }[]) {
    ...
    const manageableKeys = new Set([
      ...
      "share.shareIdLength",
      "share.maxSize",
      "share.zipCompressionLevel",
      "share.chunkSize",
      "share.autoOpenShareModal",
      "s3.enabled",
      "s3.endpoint",
      "s3.region",
      "s3.bucketName",
    ...
    if (data.some((variable) => !manageableKeys.has(variable.key))) {
      throw new BadRequestException(
        "This configuration is managed by 起航实验室",
      );
    }
