...
if [ "$(id -u)" -ne 0 ]; then
  echo "Run with sudo: sudo sh deploy/qihang-recruit-media-2/deploy.sh" >&2
  exit 1
fi
...
for required in \
  ...
  "${SOURCE_DIR}/deploy/qihang-recruit-media-2/Dockerfile" \
  "${SOURCE_DIR}/deploy/qihang-recruit-media-2/docker-compose.yml"; do
  if [ ! -e "$required" ]; then
...
docker build \
  -f "${SOURCE_DIR}/deploy/qihang-recruit-media-2/Dockerfile" \
