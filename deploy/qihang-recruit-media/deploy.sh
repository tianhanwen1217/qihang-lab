...
RELEASE="20260821-recruit-media-1"
...
if [ "$(id -u)" -ne 0 ]; then
  echo "Run with sudo: sudo sh deploy/qihang-recruit-media/deploy.sh" >&2
  exit 1
fi
...
for required in \
  ...
  "${SOURCE_DIR}/deploy/qihang-recruit-media/Dockerfile" \
  "${SOURCE_DIR}/deploy/qihang-recruit-media/docker-compose.yml"; do
  if [ ! -e "$required" ]; then
...
case "$CURRENT_IMAGE" in
  "$BASE_IMAGE"|qihang-lab-custom:20260813-upload-fix-1|qihang-lab-custom:20260813-upload-fix-2|qihang-lab-custom:20260813-upload-fix-3|qihang-lab-custom:20260813-upload-fix-4|qihang-lab-custom:20260813-upload-fix-5|qihang-lab-custom:20260815-theme-1|qihang-lab-custom:20260815-stability-1|qihang-lab-custom:20260815-harness-1)
...
docker build \
  -f "${SOURCE_DIR}/deploy/qihang-recruit-media/Dockerfile" \
