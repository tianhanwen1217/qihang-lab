...
RELEASE="20260824-package-link-1"
...
if [ "$(id -u)" -ne 0 ]; then
  echo "Run with sudo: sudo sh deploy/qihang-recruit-media-5/deploy.sh" >&2
  exit 1
fi
...
for required in \
  ...
  "${SOURCE_DIR}/deploy/qihang-recruit-media-5/Dockerfile" \
  "${SOURCE_DIR}/deploy/qihang-recruit-media-5/docker-compose.yml"; do
  if [ ! -e "$required" ]; then
...
docker build \
  -f "${SOURCE_DIR}/deploy/qihang-recruit-media-5/Dockerfile" \
